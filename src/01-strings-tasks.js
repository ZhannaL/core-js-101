...
function extractNameFromTemplate(str) {
  return str.replace(/Hello, /i, '').slice(0, -1);
}
...
function getRectangleString(width, height) {
  ...
  const rectangle = topLine + body + bottomLine;
  return rectangle;
}
 ...
 *
 */
const codeAtcharToROT13 = (char) => (char.match(/[a-z]/)
  ? ((char.charCodeAt(0) - 97 + 13) % 26) + 97
  : ((char.charCodeAt(0) - 65 + 13) % 26) + 65);

function encodeToRot13(str) {
  const arr = str.split('');
  const newArr = arr.map((el) => String.fromCharCode(
    el.match(/[\W\d]/) ? el.charCodeAt(0) : codeAtcharToROT13(el),
  ));
...
function getCardId(card) {
  ...
  const deck = suits.reduce(
    (deckinner, elSuit) => deckinner.concat(rank.map((elRank) => elRank + elSuit)),
    [],
  );
