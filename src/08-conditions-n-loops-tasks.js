...
function doRectanglesOverlap(rect1, rect2) {
  return (
    rect1.top <= (rect2.top + rect2.width) && (rect1.top + rect1.width) >= rect2.top)
    && (rect1.left <= (rect2.left + rect2.height) && (rect1.left + rect1.height) >= rect2.left
    );
}
...
function isInsideCircle(circle, point) {
  ...
  + (point.y - circle.center.y) ** 2) < circle.radius * circle.radius
  );
}
...
function getCommonDirectoryPath(paths) {
  const arrOfSplitedPathes = paths.map((path) => path.split('/').filter((el) => el !== '')).sort();
  const commonPath = arrOfSplitedPathes[0].filter(
    (el) => arrOfSplitedPathes.every((path) => path.includes(el)),
  );
  if (commonPath.length !== 0) {
    return '/'.concat(commonPath.join('/')).concat('/');
  }
  if (paths.every((el) => el.startsWith('/'))) return '/';
  return '';
}
 ...
 *
 */
const multiple2arr = (arr1, arr2) => arr1.reduce((acc, el, ind) => acc + (el * arr2[ind]), 0);
const invertMatrix = (arr) => {
  const inver = new Array(arr[0].length).fill(0).map(() => new Array(arr.length).fill(0));
...
function getMatrixProduct(m1, m2) {
  ...
  const resultArray = m1.reduce((res, row) => {
    res.push(
      invertedM2.reduce((acc, rowm2) => acc.concat(multiple2arr(rowm2, row)), []),
    );
    return res;
  }, []);
  return resultArray;
...
function evaluateTicTacToePosition(position) {
  ...
  if (typeof position[0][2] === 'string' && position[0][2] === position[1][1]
